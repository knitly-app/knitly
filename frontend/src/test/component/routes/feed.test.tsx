...
describe("FeedRoute — post reaction", () => {
  it("clicking a reaction emoji calls the reaction mutation", async () => {
    const currentUser = { id: "u1", username: "ada", displayName: "Ada", createdAt: "2024-01-01" };
    const posts = [makePost({ id: "p1", userId: "u2", content: "React to me" })];
    const qc = makeQueryClient();
    qc.setQueryData(queryKeys.auth.me(), currentUser);
    qc.setQueryData(queryKeys.circles.all(), []);
    qc.setQueryData(queryKeys.feed.byCircle(undefined), {
    ...
    await renderWithProviders(<FeedRoute />, { path: "/", queryClient: qc });
    await waitFor(() => expect(screen.getByText("React to me")).toBeInTheDocument());

    // Open the reaction picker
    const reactBtn = screen.getByLabelText("React to post");
    fireEvent.click(reactBtn);

    // Click the first emoji button (Love ❤️ — button with title "Love")
...
describe("FeedRoute — post edit", () => {
  it("clicking edit then saving calls the edit mutation", async () => {
    const currentUser = { id: "u1", username: "ada", displayName: "Ada", createdAt: "2024-01-01" };
    const posts = [makePost({ id: "p1", userId: "u1", content: "Original content" })];
    const qc = makeQueryClient();
    qc.setQueryData(queryKeys.auth.me(), currentUser);
    qc.setQueryData(queryKeys.circles.all(), []);
    qc.setQueryData(queryKeys.feed.byCircle(undefined), {
    ...
    await renderWithProviders(<FeedRoute />, { path: "/", queryClient: qc });
    await waitFor(() => expect(screen.getByText("Original content")).toBeInTheDocument());

    const editBtn = screen.queryByLabelText("Edit post");
    if (editBtn) {
      fireEvent.click(editBtn);
      // The textarea should appear with the current content
      await waitFor(() => {
